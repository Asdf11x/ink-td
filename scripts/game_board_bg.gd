...
func _draw() -> void:
	var size: Vector2 = GameConstants.GRID_PIXEL_SIZE
	draw_rect(Rect2(Vector2.ZERO, size), GameConstants.PARCHMENT, true)

	for x in range(GameConstants.GRID_COLS + 1):
		var px: float = float(x * GameConstants.TILE_SIZE)
		draw_line(Vector2(px, 0.0), Vector2(px, size.y), GameConstants.GRID_LINE, 1.0)

	for y in range(GameConstants.GRID_ROWS + 1):
		var py: float = float(y * GameConstants.TILE_SIZE)
		draw_line(Vector2(0.0, py), Vector2(size.x, py), GameConstants.GRID_LINE, 1.0)
...
func _draw_portal(cell: Vector2i, color: Color, label: String) -> void:
	...
	var rect := Rect2(
		Vector2(float(cell.x) * tile, float(cell.y) * tile),
		Vector2(tile, tile)
	)
